...
def read_table(url):
    ...
    # Remove the previous "DASH_Builder_Export.csv" file.
    if os.path.exists("DASH_Builder_Export.csv"):
        os.remove("DASH_Builder_Export.csv")
    else:
...
def csv_to_database():
    ...
    path= os.getcwd()+"\\DASH_Builder_Export.csv"
    print (path+"\\")
    path = path.replace('\\', '/')
